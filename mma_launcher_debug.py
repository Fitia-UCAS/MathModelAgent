...
2) 将所有关键控制台输出合流写入 backend/logs/launcher_debug/：
   - NNN.log        : 纯文本（去色）
   - NNN.ansi.log   : 保留 ANSI 颜色码（可选，默认开启）
   其中 NNN 与 backend/project/work_dir 的 task_id（数字目录）保持一致：按当前最大目录号 + 1 生成；
   若 work_dir 不存在或为空，退回扫描 backend/logs/messages 的编号；仍无则退回扫描 launcher_debug/*.log。
3) 仍保持实时回显到当前终端（带颜色）；
...
class _GlobalFileLogger:
    ...
    backend/logs/launcher_debug/ 下生成：
      - 001.log        : 去色纯文本
    ...
    编号策略（与 task_id 一致）：
      ...
      3) 退回 backend/logs/launcher_debug 下 *.log 的编号自增。
    """

    def __init__(self, project_root: Path):
        self.log_dir = project_root / "backend" / "logs" / "launcher_debug"
...
class PySourceDumper:
    ...
    def write_backend_app_snapshot(project_root: Path, out_txt: Path):
        ...
        文件名应形如：源码快照_NNN.txt（NNN 与本次 launcher_debug 日志编号一致）
        """
